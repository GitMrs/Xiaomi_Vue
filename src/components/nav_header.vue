...
<script>
import { mapState, mapActions } from "vuex"; // 使用mapState
export default {
  name: "nav_header",
  data() {
    return {
  ...
  mounted() {
    ...
    console.log(params);
    params && params.from == "login" && this.getCartCountNum();
  },
  computed: {
  ...
  methods: {
    ...mapActions(["saveUserName", "getCartCount"]),
    getProductList() {
      this.axios
        .get("/products", {
          params: {
